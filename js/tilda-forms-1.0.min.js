  ...
  (window.tildaForm.validate = function (form) {
    ...
    for (
      ...
      var isEmpty = !value.length && !valueNoSpace.length,
        ...
        isChecked = !form.querySelectorAll('[name="' + inputName + '"]:checked')
          .length;
    ...
    if ("y" === dataFormCart) {
      var isMinOrderSetted =
          void 0 !== window.tcart_minorder && window.tcart_minorder > 0,
        isMinQuantitySetted =
          void 0 !== window.tcart_mincntorder && window.tcart_mincntorder > 0,
        objError,
        objError;
      if (isMinOrderSetted)
...
function t_forms__calculateInputsWidth(recId) {
  ...
  if (rec) {
    var inputs = rec.querySelectorAll(".t-input-block_width");
    if (0 !== inputs.length) {
      ...
      if (formWrapper) {
        ...
        Array.prototype.forEach.call(inputs, function (input) {
          ...
          if (
            inputGroup &&
            inputGroup.classList.contains("t-input-group_inrow-withsibling")
          ) {
            var nextInputGroup = inputGroup.nextElementSibling;
            if (
              !nextInputGroup ||
              !nextInputGroup.classList.contains("t-input-group_inrow")
            )
              return;
            nextInputGroup.classList.add("t-input-group_inonerow");
          } else inputGroup.classList.add("t-input-group_inrow-last");
          var marginLeft = 15,
            columns = {
          ...
          for (var i in columns)
            if (input.classList.contains(columns[i])) {
              input.style.width = (formWrapperWidth - 15 * (i - 1)) / i + "px";
              var label = input.previousElementSibling;
              label &&
                label.classList.contains("t-input-title") &&
                (label.style.width =
                  (formWrapperWidth - 15 * (i - 1)) / i + "px"),
                window.innerWidth <= 480 &&
                  ((input.style.width = "100%"), (label.style.width = "100%"));
            }
        });
      }
    }
